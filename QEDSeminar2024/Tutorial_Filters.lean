...
Mostly we are familiar with convergence of sequences: Suppose `X` is a topological space, and `f` a sequence in `X`, and `x ∈ X`.  We say that `f` converges to `x` if for any open set `U ∋ x`, there exists an `N` such that if `n > N`, then an `x n ∈ U`. Another way of saying the same thing: for any open set `U ∋ x`, no matter how small, the sets `F_N := {x n | n > N}` are eventually contained in `U`.
...
If `X` is a type, a filter `F : Filter X` on `X` is a collection of sets `F.sets : Set (Set X)` satisfying the following axioms:
...
Example:  If `f` is a sequence in `X`, the elementary filter associated with `f` is the set
 `{A ⊂ X | ∃ N, n > N → f n ∈ A}`

We say that a filter `F` in `X` converges to a point `x : X`, if every open neighbourhood of `x` is in `F`. Think about what this means in the case of the elementary filter.
-/
section Filter
...
it is generated by the collection of up-sets `{b | a ≤ b}`, e.g.
